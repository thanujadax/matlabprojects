function [IOut,output] = denoiseImageKsvdReuseDict(Image,sigma,K,bb,Dictionary,varargin)
% Modified version ofdenoiseImageKSVD (Elad & Aharon) by - Thanuja
% Modification log:
%   added bb to the parameter list
...
%                   bb - patch size
%                   Dictionary - dictionary to be (re)used for denoising
%    Optional arguments:              
...
% 'blocks' contain the pixel values of the image - each column being an entire bb-patch 
if (waitBarOn)
...
count = 1;
Weight = zeros(NN1,NN2); % will accumulate the number of times a pixel overlaps
% while the image patches are being added up to formulate IMout
IMout = zeros(NN1,NN2);
