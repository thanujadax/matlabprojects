function [Dictionary,output,CoefMatrix] = KSVD2LNS(...
    Data,... % an nXN matrix that contins N signals (Y), each of dimension n.
    CoefMatrix,... % an nXN matrix that contins N signals (Y), each of dimension n.
    param)
% =========================================================================
%                          K-SVD algorithm
% =========================================================================
...
for iterNum = 1:param.numIteration
    ...
    elseif (param.errorFlag==3)
        %    CoefROMP
        param.testMaxAtoms = true;
        param.card = 1;
        param.errConstraint = 1;
        param.coeffThres = param.coeffFact*sqrt(size(Dictionary,1))*param.errorGoal;
%         fprintf('KSVD2LNS: addK,addX,coeffFact,coeffThres= %g,  %g, %g, %g \n', ... 
%             param.addK,param.addX,param.coeffFact,param.coeffThres);
        [~, CoefMatrix] = CoROMP3([FixedDictionaryElement,Dictionary] , Data , CoefMatrix , param);
    else 
...
%   Modify to not reuse same data element repeatedly
    ...
    CoefMatrix(j,:) = 0;
    CoefMatrix(:,i) = 0;
    CoefMatrix(j,i) = sign(Data(1,i))*norm(Data(:,i));
    NewVectorAdded = 1;
    ErrorNormVec(i) = 0;
    return;
end
